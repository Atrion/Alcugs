...
/** Important RULES to the correct operation of the NetCore

	You must never touch the session flag in any part of the code,
	please always use the DestroySession() that will do the job
	that you want.

*/

/*
	TODO: Currently there isn't any limit in the maxium number of messages that can
	be stored in the incoming and outcomming buffers.
	If things are working well, and someone tryes to exploit this from a client, the
	UNET_FLOOD event will be toggled and all associated data structures deleted disconnecting
	the player. If a slow attack if performed, it is not going to be effective, because,
	the server will always parse and delete several packets per second.
	About the send buffer, as soon as a message has been resend more than 5 times, a
	UNET_TIMEOUT event will be toggled destroying all associated data structures discnnecting
	the player.
	So, we suppose that the server is not going to eat all the free available memory.
	But, some limits will be implemented, but we already know that these limits are already
	causing problems in urunet 2.
*/
