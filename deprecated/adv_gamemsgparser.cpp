...
#ifndef __ADVGAMEMSG_SPARSER_S
#define __ADVGAMEMSG_SPARSER_S

#define __ADVGAMEMSG_SPARSER_S_ID "$Id$"
...
void dumpUruObjectDesc(FILE * f,st_UruObjectDesc * o) {
	fprintf(f,"UruObjectDesc:[flag:%i,pid:%08X,t:%04X,ot:%04X,n:%s,(ide:%i,id:%i)]",\
	o->flag,o->page_id,o->page_type,o->type,o->name,o->ide,o->id);
}
...
int compareUruObjectDesc(FILE * f,st_UruObjectDesc * a,st_UruObjectDesc * b) {
	if(a->flag!=b->flag || a->page_id!=b->page_id || a->page_type!=b->page_type ||
		strcmp((char *)a->name,(char *)b->name) || a->ide!=b->ide || a->id!=b->id) {
		return -1;
	}
	return 0;
}
...
#if 0
int adv_game_msg_parser(int sock,Byte * buf,int size,st_uru_client * u) {
	st_uru_game_msg msg;
	int off=0;
	int ret=0;
	msg.unk1=*(U32 *)(buf+off); //always 0
	off+=4;
	msg.flag=*(Byte *)(buf+off); //always 0, seen 0x03 (sometimes)
	off++;
	msg.size=*(U32 *)(buf+off); //the msg size
	off+=4;
	msg.type=*(U16 *)(buf+off); //the msg type
	off+=2;

	print2log(f_uru,"MSG:Unk1:%i,flag:%i,size:%i,type:%04X\n",msg.unk1,msg.flag,msg.size,\
msg.type);

	if(msg.unk1!=0) {
		print2log(f_une,"Unexpected unk1:%08X in adv_game_msg\n",msg.unk1);
		ret=-1;
	}
	if(msg.flag!=0) {
		print2log(f_une,"Unexpected flag:%02X in adv_game_msg\n",msg.flag);
		ret=-1;
	}
...
#if 0
extern st_uru_client * all_players;
